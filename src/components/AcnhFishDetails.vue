<template>
  <div>
    <van-panel>
      ...
      <van-cell title="賣價 (俞司廷)">{{ item['price-cj'] | commafy }}</van-cell>
      <van-cell title="魚影大小">{{ item.shadow }}</van-cell>
      <van-cell title="出沒地點">{{ availability.location }}</van-cell>
      <van-cell
        title="出沒月份 (北半球)"
      >{{ availability.isAllYear ? '全年' : availability['month-northern'] }}</van-cell>
      <van-cell
        title="出沒月份 (南半球)"
