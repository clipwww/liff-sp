<template>
  <div>
    <van-panel>
      ...
      <van-cell title="種族">{{ item.species | translateSpecies }}</van-cell>
      <van-cell title="個性">{{ item.personality }}</van-cell>
      <van-cell title="生日">{{ item.birthday | formatBirthday }}</van-cell>
      <van-cell title="喜歡的歌曲">{{ villager.favoriteSong }}</van-cell>
      <van-cell title="興趣">{{ villager.hobby }}</van-cell>
      <van-cell title="口頭禪">{{ item['catch-phrase'] }}</van-cell>
