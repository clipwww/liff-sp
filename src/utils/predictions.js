function randfloat(arr, min, max, basePrice, cb = v => v) {
  arr.push(
    [min, max]
...
function filterByPattern(filters) {
  return pattern =>
...
function possiblePatterns(filters) {
...
function averageReducer(prev, current) {
  return prev.map(([avg, count, flag], i) => {
    const [min, max] = current[i]
    if (!flag) {
      return [(avg + count + min + max) / 4, 4, true]
    }
...
function minWeekReducer(prev, current, i) {
  const [a] = current.reduce(maxReducer)
  const [b] = i === 1 ? prev.reduce(maxReducer) : prev
  return [Math.min(a, b)]
}

function patternReducer(patternsCategories, reducer = minMaxReducer) {
